...
这个论文是基于`Replicated State Machine`来实现的。`State Transfer`的复制是复制内存，而`Replicated State Machine`的复制是复制外部操作。
...
[tu](https://thepatterraining.github.io/images/6824/2-3.png)
...
时间实际上通过GPS卫星同步的，同步以后再同步给服务器，这个传播是有时间的，因此，这个时间是有不确定性的。

因此，Spanner返回一个时间对，分别是一个开始时间和一个结束时间，比如12:01-12:02.
...
有两条规则解决这个问题
- Start Rule
	- TS = TT.now().latest. 也就是时间对里面的结束时间。
	- 对于只读事务来说，分配的是开始时间
	- 对于读写事务来说，分配的是提交时间
- Commit wait Rule
	- 对于读写事务来说，必须延迟，TS < TT.now().earliest
...
之所以讨论这个，这是关于事务，复制和分片系列论文的最后一个，这仍然是一个开放的研究领域，人们对现有的性能，或性能和一致性的权衡并不满意。

Spanner 更关注地理复制，跨地域的复制，为了解决两阶段提交的问题，对于只读事务采取了一种优化。

FaRM是一个研究原型，目标是探索新型RDMA告诉网络硬件的潜力，假设所有副本在同一个数据中心内。容错能力仅限于单个崩溃。它使用了RDMA技术，因此，被迫使用了OCC。性能远高于Spanner，可以在58微秒内完成一个事务。比Spanner快了大约100倍。

Spanner的瓶颈是网络延迟，FaRM的瓶颈是服务器的CPU时间。

FaRM有一个配置管理组件，通过Zookeeper实现。按照key来进行分片到主备服务器。分片只要有一个副本存在就可以恢复，因此，如果有F+1个副本，可以容忍F次故障。

还有一堆事务客户端，他们还充当事务协调器。
...
- 使用NVRAM，一种非易失性内存
- RDMA技术
- Kernel ByPass （内核旁路），这意味着应用程序可以直接访问网卡，不需要内核介入。

### 非易失性内存

这是直接影响设计其余部分的主题。

主要思路是使用一块电池，当断电以后，使用电池供电，同时触发警报，将内存中的数据传送到一个移动硬盘中。

当数据中心的供电恢复以后，在将数据从硬盘中恢复到内存。

但是，如果是硬件故障或者是软件故障，触发机器重启，那么就无法恢复了，这不是非易失性内存可以解决的问题。

> 这就是为什么还有分片和副本的原因。

这解决了磁盘的瓶颈，但是还有网络和CPU的瓶颈。

### Kernel Bypass

通常，网络发送很繁琐，需要调用内核，中断，DMA等。在经过TCP协议等进行发送。

这个技术的思想是，应用程序直接和网卡进行通信，操作网卡发送数据，省去了中间的开销过程，极大的提升了性能。但是，对于TCP这些协议实现的重传、可靠性等等需要自己实现。这可以使用DP,DK这种包来实现。

问题：这需要修改操作系统吗？
```
我并不知道，这是运行在Win上的，我不清楚这个问题。对于Linux来说，这是支持的，这也确实需要操作系统支持，内核来授权应用程序直接和网卡通信。
```

这同样需要网卡支持，对于现代的网卡来说，基本都支持了。

### RDMA（远程直接内存访问） Scheme

这里指的是一个特殊的网卡。支持RDMA的网卡。

总的来说，这个可以和`Kernel Bypass`一起使用，RDMA的作用是让电脑A的应用程序可以直接读写电脑B的内存数据。读写请求通过电脑A的网卡发送到电脑B的网卡，然后直接操作电脑B的内存。这就是单向RDMA。

RDMA网卡通常有类似TCP的能力，会不断发送请求并确认。

这种能力只能在局域网中使用，没有办法跨越数据中心来远程读取写入。

使用RDMA进行读写的延迟约为5微秒。

### OCC

因为使用了RDMA，这个不支持锁，因此，对于并发控制来说，只能使用乐观并发控制（OCC）来完成。
...
这一步读取的时候，通过单边RDMA读取，速度非常快。

FaRM做为一个原型，不支持SQL，因此，提供了一些API来支持事务：
- txCreate(): 创建事务
- txRead(OID): 读取事务：o=txRead(OID), OID是对象ID，要读取某个对象，OID有两部分，一个是Region，代表哪个服务器，一个是具体的内存地址。
- 修改数据：o.f +=1
- txWrite() 写入事务
- txCommit() 提交事务
...
每个对象是存储在内存中的，他们有一个对象头。对象头中的高位是锁标志，低位是版本号。

Commit阶段也分成不同的阶段
- Lock： 对于要写入的数据，会进行Lock请求。这是用CAS操作来保证原子性。
- Validation：验证阶段，这里有一个优化，当只读事务的时候，会使用RDMA从目标加载对象头，然后在事务协调器里面进行比较，这比锁快的多。
- Commit Backup
- Commit Primary
- Truncate
...
假设现在有T1，T2，需要更新X的值，X当前为0，那么可能得值有：
- X=0：两个都失败
- X=1：一个成功
- X=2：两个成功

T1: Read X, Lock X, Commit
T2: Read X, Lock X, Commit
...
结果：X=1， T2失败，因为T1提交以后，版本号变更，因此，T2 Lock以后，失败。
...
执行：
T1: Read X, Lock Y 写入Y，因此LockY，Vlidation X, X只读，所以RDMA加速进行Validation。 Commit
T2: Read y, Lock X, Validation Y, Commit

这里两个程序都会终止，因为Validation阶段会看到被锁住了。

T1: Read X, Lock Y, Validation X, 			Commit
T2: Read X							Lock X, Validation Y， Commit

这里T1会成功，T2失败，因为T2Validation的时候，Y的版本变了。
